...
#region Upload
        ...
        static bool ProcessItemsUpload(WorkshopType type, List<string> paths, Options options)
        {
            ...
            {
                var mod = new Uploader(type, Path.GetFullPath(paths[idx]), options.ModNames[idx], options.Tags, options.ExcludeExtensions, options.Compile, options.DryRun, options.Development, options.Visibility, options.Force);
